...
def run(parser, args):
    ...
    if args.gapfill:
        ...
        cmds.append("align_trim %s --start --remove-incorrect-pairs --report %s.alignreport.txt < %s.sorted.bam 2> %s.alignreport.er | samtools sort -T %s - -o %s.trimmed.rg.sorted.bam" % (bed, args.sample, args.sample, args.sample, args.sample, args.sample))
        cmds.append("align_trim %s --remove-incorrect-pairs --report %s.alignreport.txt < %s.sorted.bam 2> %s.alignreport.er | samtools sort -T %s - -o %s.primertrimmed.rg.sorted.bam" % (bed, args.sample, args.sample, args.sample, args.sample, args.sample))
