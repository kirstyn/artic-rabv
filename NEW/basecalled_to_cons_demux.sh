...
usage()
{
    echo "usage: <command> run_name path_to_fast5 barcode_kit primer_scheme scheme_version fastq"
}
if [ -z "$*" ]
then
   usage
...
# find $fastq/* -maxdepth 0 -type d ! -name "*unclassified*" |
...
artic minion --no-frameshifts --medaka --medaka-model r941_min_high_g351 --normalise 200 --threads 4 --scheme-directory /home/artic/Github/artic-rabv/primer-schemes --read-file $f $scheme $sample/$sample
