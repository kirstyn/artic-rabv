...
usage()
{
    echo "usage: <command> run_name path_to_fast5"
}
if [ -z "$*" ]
then
   usage
...
#fast basecalling
guppy_basecaller -c dna_r9.4.1_450bps_fast.cfg -i $path_to_reads -s $run_name"_guppy" -r
#-x auto
...
mkdir -p $sample
#rename files
artic minion --medaka --normalise 200 --threads 4 --scheme-directory ~/Github/artic-rabv/primer-schemes --read-file $f rabvTanzDg/V2 $sample/$sample
